...
def keepintofolder(feature_train, label_train):
    checklists = ["Apples", "Bananas"]
    total = 0
    for name in checklists:
        paths = 'E:/Github/Color-Histogram-by-Python/storage/'+ name +'/sum/'
        for bases, dirs, files in os.walk(paths):
            for Files in files:
                total += 1
        print(total)

        for count in t.tqdm(range(1, total+1)):
            path = "E:/Github/Color-Histogram-by-Python/storage/%s/sum/%.02d.jpg" % (name, count)
            ...
            feature_keep = (np.array([feature])).reshape(1, 180)
            print(np.shape(feature_keep))
            classifier = sn.KNeighborsClassifier(n_neighbors=5)
            classifier.fit(feature_train, label_train)
            out = classifier.predict([feature_keep])
...
train()
# แค่หารูปเหมือนกับต่าง mean square err